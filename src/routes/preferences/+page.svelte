<script lang="ts">
  ...
  function handleStopSelect(stop: TTCStop) {
    ...
    toast.success(
      $_("toasts.addedRoutesFromStop", {
        values: { count: stop.routes.length, name: stop.name },
      })
    );
  }
  ...
  const transportModes = [
    ...
    { id: "bus", icon: "🚌", name: "Bus", description: "All bus routes" },
    {
      ...
      icon: "🚋",
      name: "Streetcar",
      description: "All streetcar routes",
    },
  ];
  ...
  const alertTypes = [
    {
      ...
      description: "No service or major outages",
    },
    {
      id: "delay",
      icon: "⏱️",
      name: "Delays",
      description: "Service delays and slowdowns",
    },
    {
      id: "detour",
      icon: "↩️",
      name: "Detours",
      description: "Route changes and detours",
    },
    {
      id: "planned",
      icon: "🛠️",
  ...
  async function handleReset() {
    if (confirm("Are you sure you want to reset all preferences?")) {
      ...
      nextScheduleId = 2;
      toast.success($_("toasts.preferencesReset"));
    }
  }
  ...
  const step3Complete = $derived(
    schedules.length > 0 && schedules.every((s) => s.days.size > 0)
  );
...
<main class="content-area">
  ...
  >
    ...
    <div class="card">
      <Accordion.Root type="single" bind:value={openAccordion}>
        <Accordion.Item value="step-1" class="border-0">
          <Accordion.Trigger
            class="w-full px-4 sm:px-6 py-4 hover:no-underline"
          >
            <div class="flex items-center gap-2 sm:gap-3 flex-1 min-w-0">
              <div
                class="flex h-7 w-7 sm:h-8 sm:w-8 shrink-0 items-center justify-center rounded-full {step1Complete
                  ? 'bg-primary'
                  : 'bg-muted'} text-{step1Complete
                  ? 'primary-foreground'
                  : 'muted-foreground'} text-xs sm:text-sm font-semibold"
              >
                {#if step1Complete}
                  <Check class="h-4 w-4" />
                {:else}
              ...
              <div class="text-left min-w-0">
                ...
                <div class="text-xs text-muted-foreground hidden sm:block">
                  Choose which TTC services you use
                </div>
              </div>
            </div>
      ...
      <Accordion.Root type="single" bind:value={openAccordion}>
        <Accordion.Item value="step-2" class="border-0">
          ...
          >
            <div class="flex items-center gap-2 sm:gap-3 flex-1 min-w-0">
              <div
                class="flex h-7 w-7 sm:h-8 sm:w-8 shrink-0 items-center justify-center rounded-full {step2Complete
                  ? 'bg-primary'
                  : 'bg-muted'} text-xs sm:text-sm font-semibold"
              >
                {#if step2Complete}
              ...
              <div class="text-left min-w-0">
                ...
                <div class="text-xs text-muted-foreground hidden sm:block">
                  Choose specific routes for alerts
                </div>
              </div>
            </div>
            <div class="flex items-center gap-1 sm:gap-2 shrink-0">
          ...
          <Accordion.Content>
            <div class="px-4 sm:px-6 pb-4">
              <!-- Search -->
              <div class="mt-4 mb-4">
                <label for="route-search" class="sr-only"
                  >{$_("search.searchRoutes")}</label
                >
                <div class="relative">
                  <Search
              ...
              <div class="max-h-64 overflow-y-auto p-1">
                <div class="flex flex-wrap gap-3">
                  {#each filteredRoutes as { route }}
                    ...
                    <button type="button" onclick={() => toggleRoute(route)}>
                      <RouteBadge
                        {route}
                        selectable={true}
                        selected={isSelected}
    ...
    <div class="card relative overflow-hidden">
      <div
        class="absolute top-0 right-0 bg-primary text-primary-foreground text-xs font-semibold px-2 py-0.5 rounded-bl"
      >
        Beta
      </div>
      <div class="px-4 sm:px-6 py-4">
        <div class="flex items-center gap-2 sm:gap-3 mb-4">
          <div
            class="flex h-7 w-7 sm:h-8 sm:w-8 shrink-0 items-center justify-center rounded-full bg-muted text-muted-foreground text-lg"
          >
            🚏
          </div>
          <div>
      ...
      <Accordion.Root type="single" bind:value={openAccordion}>
        <Accordion.Item value="step-3" class="border-0">
          ...
          >
            <div class="flex items-center gap-2 sm:gap-3 flex-1 min-w-0">
              <div
                class="flex h-7 w-7 sm:h-8 sm:w-8 shrink-0 items-center justify-center rounded-full {step3Complete
                  ? 'bg-primary'
                  : 'bg-muted'} text-xs sm:text-sm font-semibold"
              >
                {#if step3Complete}
              ...
              {#each schedules as schedule, i (schedule.id)}
                <div class="card mb-4">
                  <div class="p-4">
                    ...
                    <div class="mb-4">
                      <span
                        class="label mb-2 block text-sm font-medium"
                        id="days-label-{schedule.id}">Days</span
                      >
                      <div
                        class="flex flex-wrap gap-2"
                        role="group"
                        aria-labelledby="days-label-{schedule.id}"
                      >
                        {#each days as day}
      ...
      <Accordion.Root type="single" bind:value={openAccordion}>
        <Accordion.Item value="step-4" class="border-0">
          ...
          >
            <div class="flex items-center gap-2 sm:gap-3 flex-1 min-w-0">
              <div
                class="flex h-7 w-7 sm:h-8 sm:w-8 shrink-0 items-center justify-center rounded-full {step4Complete
                  ? 'bg-primary'
                  : 'bg-muted'} text-xs sm:text-sm font-semibold"
              >
                {#if step4Complete}
            ...
            <div class="flex items-center gap-1 sm:gap-2 shrink-0">
              <span class="badge badge-secondary text-xs"
                >{step4Complete
                  ? `${selectedAlertTypes.size} selected`
                  : "Required"}</span
              >
            </div>
          </Accordion.Trigger>
              ...
              <div class="grid grid-cols-1 gap-3 mt-4">
                {#each alertTypes as alertType}
                  ...
                  >
                    ...
                    <div
                      class="mode-check w-5 h-5 rounded-full flex items-center justify-center transition-all shrink-0 {isSelected
                        ? ''
  ...
  <Card.Root class="mb-6 lg:hidden">
    <Card.Header>
      ...
      <Card.Description
        >Get help or share your thoughts with us.</Card.Description
      >
    </Card.Header>
  ...
  <Card.Root class="mb-6">
    <Card.Header>
      ...
      <Card.Description
        >Customize display settings for better readability.</Card.Description
      >
    </Card.Header>
    <Card.Content class="space-y-6">
      ...
      <fieldset>
        ...
        >
          {#each [{ scale: "normal" as TextScale, label: "A", size: "text-sm", name: "Small" }, { scale: "medium" as TextScale, label: "A", size: "text-base", name: "Medium" }, { scale: "large" as TextScale, label: "A", size: "text-lg", name: "Large" }] as option}
        ...
        <div
          class="flex justify-between text-xs text-muted-foreground mt-2 px-1"
        >
          <span>Small</span>
          <span>Medium</span>
          <span>Large</span>
