<script lang="ts">
  ...
  let isDisplayNameValid = $derived(
    !displayNameError && displayName.trim().length >= 2
  );
    ...
    try {
      ...
      if (result.success) {
        ...
        if (result.remainingCodes !== undefined && result.remainingCodes <= 2) {
          ...
          console.warn(
            `Only ${result.remainingCodes} recovery codes remaining`
          );
        }
      } else {
...
<Dialog.Root bind:open onOpenChange={handleOpenChange}>
  <Dialog.Content class="max-w-sm">
    ...
    <div class="space-y-4 py-4">
      ...
      {#if step === "displayName"}
        ...
        <div class="text-center pt-2">
          ...
          >
            Don't have an account? <span class="text-primary font-medium"
              >Create one</span
            >
          </button>
        </div>
      ...
      {:else if step === "biometric"}
        <div class="text-center space-y-4 py-4 animate-fade-in">
          ...
          {:else}
            <div
              class="mx-auto w-20 h-20 rounded-full bg-muted flex items-center justify-center"
            >
    ...
    {#if step === "displayName" && !$biometricsAvailable}
      ...
      >
        <strong>Note:</strong> Biometric authentication may not be available on this
        device. You may need to use a recovery code to sign in.
      </div>
    {/if}
  </Dialog.Content>
</Dialog.Root>
